...
function ReorderIcon({ dragControls, iconColor, size=24, ...props }) {

  const fillColor = iconColor || "rgba(var(--textColor-rgb), 0.8)"
  return (
    <svg
...
export const ReorderItemWithDrag = ({ item, iconColor, children, extention, keyExtractor='id', style, size, ...props }) => {
    ...
    return (
      <Reorder.Item
        ...
        style={{ boxShadow, y , ...style}}
        dragListener={false}
        dragControls={dragControls}
        {...props}
      >
        <MotionItem className={"d-f g-2 align-items-center color-inherit"}>
           {children }
          <MotionItem Tag={motion.div} className='d-f p-2'> 
            <ReorderIcon className='cursor-grap' iconColor={iconColor} dragControls={dragControls} size={size}  /> 
          </MotionItem>
        </MotionItem>
        { extention && <div>
          {extention}
